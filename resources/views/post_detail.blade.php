...
<div id="fh5co-page">
      ...
      @endphp
        <h1 id="fh5co-logo" style="margin-bottom: 0px;" ><a @if (Auth::check())
          ...
          @else
              href="{{ URL('/user')}}"
          @endif 
           @else href="{{ URL('/')}}" @endif><img src="{{ asset('assets/images/SUPERHEROINA.svg') }}" style="width: 54%;"></a></h1>
        @php
              

              if($user->id != $current_post->influencer_id ) {
        ...
        padding-left: 6px;
        padding-right: 6px;
        font-size: 14px;">@if ($user->id != $current_post->influencer_id)
    ...
    @if ($user->id != $current_post->influencer_id) 
          @php
              $username_URL_string = $other_user->username_URL;
          @endphp

          @else 
          @php
              $username_URL_string = $user->username_URL;
          @endphp
          
          @endif

          @if (strlen($username_URL_string) > 13)
              ...
              <span style="width: 10px;height: 10px;background-color: #0ed145;display: inline-block;margin-right: 5px;border-radius: 30px;vertical-align: middle;"></span><span style="display: inline-block;" class="arrow">@if ($user->id != $current_post->influencer_id) <span>@<span>{{$other_user->username_URL}}</span></span> @else <span>@<span>{{$user->username_URL}}</span></span> @endif</span></p>
          ...
          @else
            ...
            padding-left: 6px;padding-right: 6px;font-size: 14px;"><span style="width: 10px;height: 10px;background-color: #0ed145;
            display: inline-block;margin-right: 5px;
            border-radius: 30px;vertical-align: middle;"></span><span>@</span>@if ($user->id != $current_post->influencer_id) {{$other_user->username_URL}} @else {{$user->username_URL}} @endif</p> --}}
    ...
    @if (Auth::check()) 
      ...
      @else 
          ...
          @endif style="text-transform: uppercase;
            color: #222;
            font-size: 0.83em;
            font-family: consolas !important;">{{ __('nav.wall') }}</a> </li>
          <li> <a @if ($influencer->role == "influencer")
            href="{{ URL('/posts/'.$username_URL.'')}}"
          @else
          href="{{ URL('/posts/'.$username_URL.'')}}"
          @endif  style="text-decoration: underline;text-transform: uppercase;
            color: #222;
            font-size: 0.83em;
            font-family: consolas !important;">{{ __('nav.posts') }}</a> </li>
          </ul>
            ...
            button.addEventListener('click', () => {
              ...
              @if (Auth::check())
                    ...
                    @if ($user->role == "influencer")
                        ...
                        <li @if ($user->id != $current_post->influencer_id) @else class="fh5co-active" @endif><a href="{{ URL('/influencer/posts')}}">{{ __('nav.posts') }}</a></li><br>                
             ...
             <li ><a href="{{ URL('/user/feed')}}">{{ __('nav.feed') }}</a></li>
              ...
              <li style="margin-bottom: 5px; margin-top: -5px;"><span style="font-size:13px; color:#c3c3c3; font-family:  Consolas, Menlo, Monaco, Lucida Console, Liberation Mono, DejaVu Sans Mono, Bitstream Vera Sans Mono, Courier New, monospace, serif !important;">{{ __('nav.my') }}</span></li>
                        ...
                        <li @if ($user->id != $current_post->influencer_id) @else class="fh5co-active" @endif><a href="{{ URL('/user/posts')}}">{{ __('nav.posts') }}</a></li><br>
                  @endif
                ...
                <li class="username_li" style="font-size: 80%;">
                  <div class="">
                    ...
                    <div class="collapse-div collapse" id="demo" style="left: 12px;padding: 10px;position: relative;width: fit-content;float: none;box-shadow: none;border: 0px;">  <p class="dropdown-item control_panel_item"  @if(Auth::user()->username_URL == "influencer") data-target="#exampleModalCenter" @else data-target="#userModalCenter" @endif  style="margin: 0px;cursor: pointer;border: 2px solid #585858;padding: 6px;"><i class="far fa-user-circle" ></i> {{ __('nav.control_panel') }}</p>
                ...
                <li class="login_btn world_after" style="margin-top: -5px;margin-bottom: 26px;">
                  <a class="world_after" href="{{'/world'}}"><b>{{ __('nav.world') }}</b></a>
                </li>
          ...
          @if (Auth::check())
                <div class="row">
                  ...
                  <div class="col-md-12 animate-box" data-animate-effect="fadeInLeft">
                            ...
                            <div class="spotlight-group" data-fit="contain" data-autohide="all">
                              ...
                              @endphp        
                                ...
                                <a class="spotlight" data-media="video"
                                    ...
                                    data-controls="true"
                                    data-inline="true"
                                    data-page="false" data-autofit="true" >
                                 
                                <video controlsList="nodownload"  style="max-width: 100%;@if($current_post->id != $post->id) display: none; @endif"  controls loop @if($current_post->id == $post->id) autoplay @endif >
                                  <source src="{{ asset('assets/images/'.$image_video.'') }}" data-toggle="modal" data-target="#postimageModalCenter"  type="video/mp4" >
                                    
                                </video>
                                </a>
                                @else
                                <a class="spotlight" href="{{ asset('assets/images/'.$image_video.'') }}" data-page="false" data-autofit="true">
                                    <img src="{{ asset('assets/images/'.$image_video.'') }}" data-toggle="modal" data-target="#postimageModalCenter"   class="img-responsive" style="border-radius: 26px !important; border: 7px solid lightgray;max-width: 100%;max-height: 411px;@if($current_post->id != $post->id) display: none; @endif">
                                </a>    
                                @endif
                                @endforeach
                            </div>

                              {{-- <img src="{{ asset('assets/images/'.$image_video.'') }}" data-toggle="modal" data-target="#postimageModalCenter" alt="Nueva Modelo Superfans - Â¡Bienvenid@ a mi Portfolio!" class="img-responsive" style="border-radius: 26px !important;  border: 7px solid lightgray;"> --}}
                    ...
                    {{-- <center>
                      <span data-toggle="modal" data-target="#exampleModalCenter" class="btn btn-primary btn-outline"><img src="{{ asset('assets/images/SUPERHEROINA.svg') }}" style="width: 38px; margin: 3px; position: relative; left: -4px;">{{ __('content.follow') }}  <b>+</b> | <b>${{$influencer->price_of_subscription}}</b></span><br>
                    </center> --}}
                    ...
                    <center>
                        <span class="btn btn-primary btn-outline like_post"target="_blank"style="width: 292px !important;"  post_id="{{$current_post->id}}">
                            ...
                            <center><img src="{{ asset('assets/images/blue24.png') }}" alt="Nueva Modelo Superfans - Â¡Bienvenid@ a mi Portfolio!" class="img-responsive" style="border-radius: 26px !important;  border: 7px solid lightgray;">
                  ...
                  @if ($influencer->role == "influencer")
                    <center>                  
                      <span data-toggle="modal" data-target="#paymentModalCenter" class="btn-m btn btn-primary btn-outline btn-outline-default play-sound6" 
                     ><img src="{{ asset('assets/images/SUPERHEROINA.svg') }}" style="width: 38px; position: relative; left: -4px;">{{ __('content.subscribe') }} <b>+</b> {{ __('content.to_see_post') }} | <b>${{$influencer->price_of_subscription}}</b></span><br>
                    </center>
                          ...
                          <div class="modal-body" style="border: 0px">
                            ...
                            <form role="form" action="{{ route('stripe.post') }}" method="post"
                                    ...
                                    <div style="margin-bottom: 30px; ">
                                      <p style="margin: 0px;"><img src="{{ asset('assets/images/credit_card.png') }}" alt="" style="width: 50px;"> Your credit card payments will always be safe</p>
                                    </div>
                  ...
                  <form action="{{ url('/user/follow') }}" method="post" enctype="multipart/form-data">
                    @csrf                  
                      ...
                      <center>
                        <button @if(Auth::user()->id == $user->id) data-toggle="modal" data-target="#userModalCenter" type="button" @endif class="btn-m btn btn-primary btn-outline" ><img src="{{ asset('assets/images/SUPERHEROINA.svg') }}" style="width: 38px; margin: 3px; position: relative; left: -4px;">{{ __('content.follow') }}  <b>+</b> {{ __('content.to_see_post') }}</button><br>
                      </center>
                  </form>
...
<div class="spotlight-group" data-fit="contain" data-autohide="all">
  ...
  @endphp        
    ...
    @endforeach
                ...
                <div class="row work-pagination animate-box" data-animate-effect="fadeInLeft">
                  <div class="col-md-8 col-md-offset-2 col-sm-12 col-sm-offset-0">
                      ...
                      <div class="col-md-4 col-sm-4 col-xs-4 text-center">
                          ...
                          <a href="{{ URL('/influencer/post_detail/'.$next_post_id.'')}}"><i class="icon-long-arrow-left"></i> <span>Anterior</span></a>    
                          @else
                      ...
                      <div class="col-md-4 col-sm-4 col-xs-4 text-center">
                          @php
                              ...
                              $username_URL = $current_influencer->username_URL;
                          @endphp
                          <a @if (Auth::user()->id != $current_post->influencer_id)
                            ...
                            href="{{ URL('/influencer/user_posts/'.$username_URL.'')}}"
                            @else
                            href="{{ URL('/posts/'.$username_URL.'')}}"    
                            @endif
                          ...
                          @else
                            href="{{ URL('/influencer/posts')}}"
                          @endif ><i class="icon-th-large"></i></a>
                      </div>
                      <div class="col-md-4 col-sm-4 col-xs-4 text-center">
                          @if (isset($next_post_id))
                          <a href="{{ URL('/influencer/post_detail/'.$prev_post_id.'')}}"><span>Siguiente</span> <i class="icon-long-arrow-right"></i></a>    
                          @else
                ...
                <!-- Modal -->
                <div class="modal fade" id="postimageModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered" role="document" style="width: 100%;height: 100%;margin-top: 0px;">
                    <div class="modal-content" style="background: transparent;box-shadow: none;border: 0px;">
                      <div class="modal-header" style="border: 0px;padding-bottom: 50px;">
                      ...
                      <div class="modal-body">
                        <img src="{{ asset('assets/images/'.$image_video.'') }}" alt="Nueva Modelo Superfans - Â¡Bienvenid@ a mi Portfolio!" class="img-responsive" style="border-radius: 26px !important;  border: 7px solid lightgray;margin-left: auto;margin-right: auto;">
                      </div>
          ...
          @else
                ...
                <div class="row">
                  ...
                  <div class="col-md-12 animate-box fadeInLeft animated" data-animate-effect="fadeInLeft">
                    <figure class="text-center">
                      <center><!--<img src="images/l1.jpg" alt="Free HTML5 Bootstrap" class="img-responsive">--><img src="{{ asset('assets/images/blue24.png') }}" alt="Nueva Modelo Superfans - Â¡Bienvenid@ a mi Portfolio!" class="img-responsive" style="border-radius: 26px !important;  border: 7px solid lightgray;">
                  ...
                  <center>
                    {{-- href="{{url('/login')}}" --}}
                    <a @if (!Auth::check()) data-toggle="modal" data-target="#register_Modal" @endif  class="btn-m btn btn-primary btn-outline" target="_blank"  >
                      <img src="{{ asset('assets/images/SUPERHEROINA.svg') }}" style="width: 38px; position: relative; left: -7px;"> @if($user->role == "influencer") {{ __('content.subscribe') }} <b>+</b> {{ __('content.to_see_post') }} | <b>${{$user->price_of_subscription}}</b> @else {{ __('content.follow') }}  <b>+</b> {{ __('content.to_see_post') }} @endif 
                    </a>
                  </center>
                ...
                <div class="row work-pagination animate-box" data-animate-effect="fadeInLeft">
                  <div class="col-md-8 col-md-offset-2 col-sm-12 col-sm-offset-0">
                      ...
                      <div class="col-md-4 col-sm-4 col-xs-4 text-center">
                          @if (isset($prev_post_id))
                          <a href="{{ URL('/post/'.$next_post_id.'')}}"><i class="icon-long-arrow-left"></i> <span>Anterior</span></a>    
                          @else
                      ...
                      <div class="col-md-4 col-sm-4 col-xs-4 text-center">
                          @if (isset($next_post_id))
                          <a href="{{ URL('/post/'.$prev_post_id.'')}}"><span>Siguiente</span> <i class="icon-long-arrow-right"></i></a>    
                          @else
                          <p ><span>Siguiente</span> <i class="icon-long-arrow-right"></i></p>
                          @endif
