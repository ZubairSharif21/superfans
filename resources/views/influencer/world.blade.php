  ...
  .tittlei {
    ...
    <div id="fh5co-page">
        ...
        padding-left: 6px;padding-right: 6px;font-size: 14px;width: 50%;position: relative;overflow-x: hidden;overflow-y: hidden;">
            ...
            font-family: Consolas, Menlo, Monaco, Lucida Console, Liberation Mono, DejaVu Sans Mono, Bitstream Vera Sans Mono, Courier New, monospace, serif !important;">{{ __('nav.world') }}</a> </li>
            ...
            button.addEventListener('click', () => {
                document.getElementById('clickSound8').play();
                        ...
                        <hr style="margin-top: -10px;margin-bottom: 20px;">
                         ...
                         <li class="fh5co-active world_after"><a href="{{ URL('/influencer/world') }}">{{ __('nav.world') }}</a></li>
  ...
  <div style="text-align: center; margin-top: 2rem;">
    <div class="footer-language-selector" style="position: relative; display: inline-block;">
      ...
      <div class="footer-lang-menu" style="display: none; position: absolute; left: 75%; bottom: 30px; background: white; border: 1px solid #ccc; padding: 8px 12px; z-index: 10;">
        @foreach(['en', 'es', 'fr', 'de', 'it', 'pt', 'cn'] as $locale)
...
<script>
  ...
  <div class="welcome-row" style="display:flex; align-items:center; justify-content:center; gap:10px; margin-top:10px; flex-wrap:wrap;">
    ...
    <div style="display:flex; align-items:center; gap:5px; margin-left: 40px;">
      <a href="/rankings" 
         class="ranking-btn btn btn-dark"
         >
...
<div class="row animate-box" data-animate-effect="fadeInLeft" style="display: block; flex-wrap: wrap; padding: 0 1.5em;">
<!--
@php
    ...
    foreach ($posts as $post) {
        ...
        $followers_count = $influencer->no_of_followers ?? 0;
        $post_limit = $followers_count >= 15000 ? 2 : 1; 

        if (!isset($influencer_posts[$influencer_id])) {
            $influencer_posts[$influencer_id] = 0;
        }

        if ($influencer_posts[$influencer_id] < $post_limit) {
            $final_posts[] = $post;
            $influencer_posts[$influencer_id]++;
        }
    }

    $posts->setCollection(collect($final_posts));
@endphp
-->
...
<div class="col-md-4 col-sm-6 col-xs-6 col-xxs-12 work-item">
 ...
 <div class="animated-heart" id="heart-{{ $post->id }}"></div>       
  @php
    // Split the tagged profile pictures into an array
    $taggedProfilesArray = explode(',', $post->tagged_profile_pictures);
    // Split the tagged users into an array
    ...
    <button onclick="event.stopPropagation(); toggleReportOptions({{ $post->id }})"
        class="report-button report-mobile"
            style="
                ...
                display: none;
            ">
    <img src="{{ asset('assets/images/arrowDown_.png') }}" alt="Ã¢â€“Â¼" style="width: 12px; height: 12px; position:relative; bottom:1px; right:2px;">
    </button>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content" style="background:#8a2be2; box-shadow:none;">
      ...
      <div class="modal-header" style="border:none;">
        ...
        <h3 class="modal-title" style="color:#fff; font-family: Consolas, Menlo, Monaco, 'Lucida Console', monospace; font-weight:900;text-align:center;padding-top:25px">
          ✈️ {{ __('content.tourModalTitle') }}
        </h3>
      </div>

      <!-- BODY -->
      <div class="modal-body payment_modal d-block text-center">

        <p style="color:#fff;">
          {!! __('content.tourModalDescription') !!}
        </p>
     ...
     <!-- <div class="modal-footer" style="border:none; justify-content:center;">
        <button type="button" class="btn btn-light" data-dismiss="modal" style="font-weight:bold;">
          {{ __('content.close') }}
        </button>
      </div>-->
