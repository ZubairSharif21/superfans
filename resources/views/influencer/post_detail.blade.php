...
<div id="fh5co-page">
      ...
      @endphp
        ...
        @php
          ...
          @if (strlen($username_URL_string) > 13)
              ...
              <span style="width: 10px;height: 10px;background-color: #0ed145;display: inline-block;margin-right: 5px;border-radius: 30px;vertical-align: middle;"></span><span style="display: inline-block;" class="arrow">@if (Auth::user()->id != $current_post->influencer_id) <span>@<span>{{$other_user->username_URL}}</span></span> @else <span>@<span>{{$user->username_URL}}</span></span> @endif</span></p>
          ...
          @else
            ...
            border-radius: 30px;vertical-align: middle;"></span><span>@</span>@if (Auth::user()->id != $current_post->influencer_id) {{$other_user->username_URL}} @else {{$user->username_URL}} @endif</p> --}}

            <p style="margin-bottom: 0px !important;width: fit-content;margin-left: auto;margin-right: auto;
              border: 5px solid black;color: #c3c3c3;background-color: #585858;font-family: Consolas, Menlo, Monaco, Lucida Console, Liberation Mono, DejaVu Sans Mono, Bitstream Vera Sans Mono, Courier New, monospace, serif !important;
              padding-left: 6px;padding-right: 6px;font-size: 14px;width: 50%;position: relative;overflow-x: hidden;overflow-y: hidden;">
              <span style="width: 20px;height: 20px;background: #585858;position: absolute;z-index: 2;left: 0px;"></span>
              <span style="width: 10px;height: 10px;background-color: #0ed145;display: inline-block;margin-right: 5px;border-radius: 30px;vertical-align: middle;z-index: 3;position: absolute;left: 5px;top: 5px;"></span>
              <span style="width: 10px;height: 10px;background-color: #0ed145;display: inline-block;margin-right: 5px;border-radius: 30px;vertical-align: middle;"></span><span style="display: inline-block;" class="arrow_small_username">@if (Auth::user()->id != $current_post->influencer_id) <span>@<span>{{$other_user->username_URL}}</span></span> @else <span>@<span>{{$user->username_URL}}</span></span> @endif</span></p>
    ...
    @endif </p> --}}
          ...
          href="{{ URL('/influencer/influencer/'.$username_URL.'')}}"
          @else 
          href="{{ URL('/influencer/user/'.$username_URL.'')}}"
          ...
          <li> <a @if ($influencer->role == "influencer")
            href="{{ URL('/influencer/influencer_posts/'.$username_URL.'')}}"
          @else
            ...
            button.addEventListener('click', () => {
              ...
              <li class="world_after"><a href="{{ URL('/influencer/world')}}">{{ __('nav.world') }}</a></li>
               ...
               <li ><a href="{{ URL('/influencer/')}}">{{ __('nav.wall') }}</a></li>
                <li @if (Auth::user()->id != $current_post->influencer_id) @else class="fh5co-active" @endif ><a href="{{ URL('/influencer/posts')}}">{{ __('nav.posts') }}</a></li><br>
        ...
        @endif --}}
          <div class="row">
             ...
             $file_extension = pathinfo($image_video, PATHINFO_EXTENSION);
              ...
              @endphp
               ...
               @if (in_array(strtolower($file_extension), $video_extensions))
                            ...
                            @else
                            <a class="spotlight" href="{{ asset('assets/images/'.$image_video.'') }}" data-page="false" data-autofit="false">
                ...
                {{-- <center>
                  <span data-toggle="modal" data-target="#exampleModalCenter" class="btn btn-primary btn-outline" ><img src="{{ asset('assets/images/SUPERHEROINA.svg') }}" style="width: 38px; position: relative; left: -4px;">{{ __('content.follow') }}  <b>+</b> | <b>${{$influencer->price_of_subscription}}</b></span><br>
                </center> --}}
            ...
            @endphp
                ...
                <center>
                    <span class="btn btn-primary btn-outline like_post"target="_blank"style="width: 292px !important;"  post_id="{{$current_post->id}}">
                        ...
                        <center><img src="{{ asset('assets/images/blue24.png') }}" alt="Nueva Modelo Superfans - Â¡Bienvenid@ a mi Portfolio!kk" class="img-responsive" style="border-radius: 26px !important;  border: 7px solid lightgray;">
              ...
              @if ($influencer->role == "influencer")
                <center>                  
                  <span data-toggle="modal" data-target="#paymentModalCenter" class="btn btn-m btn-primary btn-outline btn-outline-default" ><img src="{{ asset('assets/images/output-onlinepngtools.png') }}" style="width: 38px; position: relative; left: -4px;">{{ __('content.subscribe') }} <b>+</b> {{ __('content.to_see_post') }} | <b>${{$influencer->price_of_subscription}}</b></span><br>
                </center>
              ...
              <form action="{{ url('/user/follow') }}" method="post" enctype="multipart/form-data">
                @csrf                  
                  ...
                  <center>
                    <button @if(Auth::user()->id == $user->id) data-toggle="modal" data-target="#userModalCenter" type="button" @endif class="btn btn-primary btn-outline" ><img src="{{ asset('assets/images/output-onlinepngtools.png') }}" style="width: 38px; position: relative; left: -4px;">{{ __('content.follow') }}  <b>+</b></button><br>
                  </center>
              </form>
            ...
            @endphp
                ...
                <center>
                    <span class="btn btn-primary btn-outline like_post"target="_blank"style="width: 292px !important;"  post_id="{{$current_post->id}}">
                        <img src="{{ asset('assets/images/output-onlinepngtools.png') }}" style="width: 15.47%; position: relative; left: -7px;"> Like <b><i class="@if($check_post_like == null) icon-heart-o @else icon-heart  @endif  post_icon_{{$current_post->id}}" aria-hidden="true"></i></b>
                    </span>
                </center>
