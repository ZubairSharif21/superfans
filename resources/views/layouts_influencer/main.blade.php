...
@php
    $themeColor = '#ffc83d'; // default: yellow
    if (auth()->check()) {
...
<meta name="theme-color" content="{{$themeColor}}">
	{{-- <title>Superworld | Nueva Modelo</title> --}}
	<title>Superworld | {{Auth::user()->username_URL}}</title>
...
<link rel="manifest" href="/manifest.json">
 ...
 /* Custom scrollbar overlay styles */
    .custom-scrollbar {
visibility: hidden;       position: fixed;
  ...
  transition: opacity 0.4s ease, visibility 0.4s ease;
    ...
    body.ios #fh5co-aside {
      scrollbar-width: none;   
      -ms-overflow-style: none;
    }
    body.ios #fh5co-aside::-webkit-scrollbar {
      display: none;          
    }
body.ios .custom-d-block {
  opacity: 0.9;       
  visibility: visible;
}
body.ios .custom-display-none {
  display: none;      
}
body.ios .custom-display-block {
  display: block;     
}
...
@media (min-width: 443px) and (max-width: 470px) {
  .custom-scrollbar {
    left: calc(58% - 10px);
  }
}
...
<body>
    ...
    @include('layouts_influencer.header')

    @yield('content')

    @include('layouts_influencer.footer')
	...
	<script>
		$(document).ready(function() {
			...
			function handleGesture() {
				...
				console.log("touchendX: "+touchendX+"  touchstartX:"+touchstartX);
	...
	<div class="modal-dialog modal-lg" role="document">
	  <div class="modal-content">
			<div class="modal-header">
			  ...
			  <p class="cropper-subtitle" style="color: #666 !important; font-family: consolas ; font-size: 0.7em;margin-top: 10px;">Estamos en <i>Beta Phase</i>. Pronto estará disponible subir imágenes HD en tu cover o foto de perfil.</p>
			</div>
...
$('.custom-scrollbar').removeClass('custom-display-block');
$('.custom-scrollbar').addClass('custom-display-none');
    let toggle = $(this);

    setTimeout(() => {
        $('.custom-scrollbar').removeClass('custom-display-none');
        $('.custom-scrollbar').toggleClass('custom-d-block', toggle.hasClass('active'));
    }, 300);
});
const toggleBtn = document.querySelector('.js-fh5co-nav-toggle');

const observer = new MutationObserver(() => {
    const isActive = toggleBtn.classList.contains('active');

    if (isActive) {
        // $('.custom-scrollbar').addClass('custom-d-block').removeClass('custom-display-none');
    } else {
        $('.custom-scrollbar').removeClass('custom-d-block').addClass('custom-display-none');
    }
});
...
observer.observe(toggleBtn, { attributes: true, attributeFilter: ['class'] });
  ...
  var aside = document.querySelector('.fh5co-aside') || document.getElementById('fh5co-aside');
  ...
  aside.parentNode.insertBefore(customScrollbar, aside.nextSibling);

  // Scroll logic
  function updateThumb() {
    var contentHeight = aside.scrollHeight;
    var visibleHeight = aside.clientHeight;
    var scrollTop = aside.scrollTop;
    var thumbHeight = Math.max((visibleHeight / contentHeight) * visibleHeight, 40);
    var thumbTop = (scrollTop / contentHeight) * visibleHeight + aside.offsetTop;
    thumb.style.height = thumbHeight + 'px';
    thumb.style.top = thumbTop + 'px';
  }
  aside.addEventListener('scroll', updateThumb);
