     ...
     let clickCount = 0;
        ...
        document.querySelectorAll('.play-sound8').forEach(button => {
            button.addEventListener('click', () => {
              ...
              <li class="fh5co-active"><a href="{{ URL('/user/feed')}}">{{ __('nav.feed') }}</a></li>
...
color: #dd2110;"><br><span>"{{ __('content.feed_of_me') }} <br> Superfans World"</span></h2></center>
    ...
    @endif>
                                ...
                                @else
                                    ...
                                    <img src="{{ asset('assets/images/'.$image_video.'') }}" alt="Nueva Modelo Superfans - Ã‚Â¡Bienvenid@ a mi Portfolio!" class="img-responsive" style="border-radius: 26px !important; border: 7px solid lightgray;width: 100%;max-height: 411px;">    
                ...
                @foreach ($subscriptions as $subscription)
                    ...
                    @php
                      ...
                      //  $user_followed = DB::table('followerships')->where('id',$followership->followed_user_id)->get();
                       ...
                       $posts = DB::table('posts')->where('influencer_id',$subscription->influencer_id)->orderBy('created_at','Desc')->take(5)->get();

                       $post_user = DB::table('users')->where('id',$subscription->influencer_id)->first();
...
@else
                          ...
                          <p href="{{url(''.$username_URL.'')}}" class="feed_posts_heading_p feed_posts_heading_pi" style="cursor:pointer; position: relative; top:25px; font-size: 14px;" onclick="event.stopPropagation();"><img class="feed_posts_heading_img" @if ($profile_picture != null)
                            src="{{ asset('assets/images/'.$profile_picture.'') }}"
                          @else
                          src="{{ asset('assets/images/output-onlinepngtools (2).png') }}"
                          @endif  alt=""><span>@</span>{{$post_user->username_URL}}</p>
...
<div class="image-hover-wrapper" style="position: relative; display: inline-block; width: 100%;">
    ...
    @endif>
                          ...
                          @else
                              ...
                              <img src="{{ asset('assets/images/'.$image_video.'') }}" alt="Nueva Modelo Superfans - Ã‚Â¡Bienvenid@ a mi Portfolio!" class="img-responsive" style="border-radius: 26px !important; border: 7px solid lightgray;width: 100%;">    
                ...
                @foreach ($followerships as $followership)
                    ...
                    @php
                      ...
                      //  $user_followed = DB::table('followerships')->where('id',$followership->followed_user_id)->get();
                       ...
                       $posts = DB::table('posts')->where('influencer_id',$followership->followed_user_id)->orderBy('updated_at','Desc')->take(5)->get();

                       $post_user = DB::table('users')->where('id',$followership->followed_user_id)->first();
...
@else
                          ...
                          <p href="{{url(''.$username_URL.'')}}" class="feed_posts_heading_p feed_posts_heading_pi"  ><img class="feed_posts_heading_img" @if ($profile_picture != null)
                            src="{{ asset('assets/images/'.$profile_picture.'') }}"
                          @else
                          src="{{ asset('assets/images/output-onlinepngtools (2).png') }}"
                          @endif  alt=""><span>@</span>{{$post_user->username_URL}}</p>
...
$likes = DB::select("SELECT * FROM posts WHERE id = ". $post->id);
                ...
                <center style="width: 100%;">
                  ...
                  <p style="text-align: center;margin-left: 20px;margin-right: 20px;"><img src="{{url('assets/images/time_icon.PNG')}}" style="margin-top: -7px;height: 32px;width: 25px;" alt=""> Los usuarios que sigues no han posteado todabÃƒÂ­a 
              ...
              <p style="text-align: center;margin-left: 20px;margin-right: 20px;"><img src="{{url('assets/images/time_icon.PNG')}}" style="margin-top: -7px;height: 32px;width: 25px;" alt=""> {{ __('content.no_post') }} 
