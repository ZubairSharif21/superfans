...
<div id="fh5co-page">
    ...
    border-radius: 30px;vertical-align: middle;"></span><span>@</span>{{$user->username_URL}}</p>
      ...
      @endphp
          ...
          <li> <a class="fh5co-active" href="{{ URL('/user/influencer/'.$username_URL.'')}}" style="text-decoration: underline;text-transform: uppercase;
          ...
          <li> <a href="{{ URL('/user/influencer_posts/'.$username_URL.'')}}" style="text-transform: uppercase;
              ...
              @endif              
                ...
                @if ($subscription === null)
                  ...
                  {{-- @dd("Auth::user()->id: ".$user_id."user->id: ".$user->id) --}}
                    ...
                    <span @if(Auth::user()->id == $user->id) data-toggle="modal" data-target="#userModalCenter" @else data-toggle="modal" data-target="#paymentModalCenter" @endif  class="btn btn-primary btn-outline" style="width: 226px !important;"><img src="{{ asset('assets/images/SUPERHEROINA.svg') }}" style="width: 17.47%; margin: 3px; position: relative; left: -4px;">{{ __('content.follow') }}  <b>+</b> | <b>${{$user->price_of_subscription}}</b></span><br>
...
{{ __('content.subscription_warning') }}
                ...
                <div class="modal" id="paymentModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" style="z-index: 123122;">
                  ...
                  <form action="<?php echo $paypal_url; ?>" method="post">			
                    ...
                    <input type='hidden' name='cancel_return' value='{{url('/user/influencer/'.$username_URL.'')}}'>
              ...
              @endphp
                ...
                <b><a href="{{ URL('/user/influencer_posts/'.$username_URL.'')}}" style="font-size: 0.69em;">{{$posts_count}}</b> {{ __('content.post') }}</a> | <b><a href="#" @if ($subscription === null) data-toggle="modal" data-target="#paymentModalCenter" @else data-toggle="modal" data-target="#LikesModalCenter" @endif  style="font-size: 0.69em;"><span class="number_of_likes">{{thousandsCurrencyFormat($likes_count)}}</span></b> {{ __('content.likes') }} </a> | <b><a href="#" @if ($subscription === null) data-toggle="modal" data-target="#paymentModalCenter" @else  data-toggle="modal" data-target="#subcriberlistmodal" @endif class="direct_subscriber_list" style="font-size: 0.69em;">{{thousandsCurrencyFormat($subscription_count)}}</b> {{ __('content.followers') }}</a>
...
<div class="modal" id="subcriberlistmodal" tabindex="-1" role="dialog" aria-labelledby="subcriberlistModalCenterTitle" aria-hidden="true" style="z-index: 1288912889;">
...
$likes = DB::select("SELECT * FROM posts WHERE id = ". $post->id);
                          ...
                          <div class="col-md-4 col-sm-6 col-xs-6 col-xxs-12 work-item" style="display: flex;align-items: center;">
                            ...
                            <center style="width: 100%;margin-bottom: 0.29em;">
                              ...
                              <a href="{{ URL('/user/influencer_posts/'.$username_URL.'')}}" class="btn btn-primary btn-outline">{{ __('content.all_posts') }}</a>
