 ...
 .feed_posts_heading_img  {
    ...
    <div id="fh5co-page">
        ...
        <aside id="fh5co-aside" role="complementary" class="border js-fullheight">
            ...
            <h1 id="fh5co-logo" style="margin-bottom: 0px;"><a href="{{ URL('/user') }}"><img
...
<hr style="margin-bottom: 20px;">
                        ...
                        <li class="fh5co-active"><a href="{{ URL('/user') }}">{{ __('nav.wall') }}</a></li>
...
{{--                                    style="font-size: 2.1em;"></i></a></li>--}}
                    ...
                    <p><small><span><a href="https://live.superfanss.app/super-ads" target="_blank" style="color:gray !important;">🏢: SUPER ADS CENTER</a> </span><br>&copy; 2021-2025 @if (Auth::user()->footer_name_username == 'username')
                            ...
                            @endif <br>{{ __('footer.all_rights_reserved') }}</span> <span>{{ __('footer.powered_by') }} <a href="https://live.superfanss.app/world">SUPERFANS WORLD S.L</a></span> </small></p>
...
<script>
 ...
 <br><b style="color: gray !important;">{{$user->name}}</b> <br /> 
                    <b>@if(!empty($user->bio))
       @php
...
<div style="max-width: 100%; word-wrap: break-word;">
  <p style="color: gray; margin-bottom: 5px;">{!! $bio !!}</p>
    ...
    @endif  <br><br>
                    ...
                    <p class="btn btn-primary btn-outline" data-toggle="modal" data-target="#userModalCenter"
                        style="width: 226px !important;"><img
                            src="{{ asset('assets/images/SUPERHEROINA.svg') }}"
                            style="width: 17.47%; margin: 3px; position: relative; left: -4px;">{{ __('content.follow') }}  <b>+</b></b></p><button onclick="window.location.href='{{ URL('/user/messages') }}'"  class="btn btn-success play-sound" style="border: 2px solid #409c40 !important; width:65px; height: 59px; display: inline-block; margin-left: 1px;">
                                 ✉️
                                    </button><br>
                    ...
                    <b><a href="{{ URL('/user/posts') }}" style="font-size: 0.69em;">{{ $posts_count }}</b> {{ __('content.post') }}</a> |
                    <b><a href="#" data-toggle="modal" data-target="" style="font-size: 0.69em;"><span
                                class="number_of_likes">
                                {{ thousandsCurrencyFormat($likes) }}
                            </span><span class="likes_value" style="display:none">{{ $likes }}</span></b> {{ __('content.likes') }} </a> |
                    <!--<b><a href="#"  data-toggle="modal" data-target="#followerlistmodal" class="direct_subscriber_list" style="font-size: 0.69em;">{{ $subscription_count }}</b> {{ __('content.followers') }}</a>-->


                    <b><a href="#" data-toggle="modal" data-target="" class="direct_subscriber_list"
                            style="font-size: 0.69em;">
                            @if ($followers_count->no_of_followers == null)
                                {{ 0 }}
                                @else{{ thousandsCurrencyFormat($followers_count->no_of_followers + $subscription_count) }}
                            @endif
...
@endif
    ...
    @endif>
                                ...
                                <center>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b><a class="post_stats"
                                        ...
                                        style="cursor: pointer;font-weight: 100;">{{ __('content.likes') }} </span></span></a> &nbsp; </b></center>
                        ...
                        <div class="col-md-4 col-sm-6 col-xs-6 col-xxs-12 work-item"
                            ...
                            <center style="width: 100%;margin-bottom: 0.29em;">
                                ...
                                @if (count($last_three_posts) > 0)
                                    <a href="{{ URL('/user/posts') }}" class="btn btn-primary btn-outline">{{ __('content.all_posts') }}</a>
                                @else
                        ...
                        @foreach ($Influencer_network_ids as $Influencer_network_id)
                            <div class="col-md-4 col-sm-6 col-xs-6 col-xxs-12 work-item">
                                ...
                                <center>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b><a
                                    ...
                                    @else
                                    src="{{ asset('assets/images/' . $socialnetwork_image . '') }}" @endif
                                                            ...
                                                            style="font-weight: normal;">{{ __('content.go_to') }}</span>
                                                        {{ $socialnetwork_name }}</b></b>
...
<br><br>
                    <p><b><a href="https://live.superfanss.app/super-ads" target="_blank" style="color:gray !important;">🏢: SUPER ADS CENTER</a> </b><br>© 2021-2025 @if (Auth::user()->footer_name_username == 'username')
                            <span style="display: inline-block;">@</span>{{ Auth::user()->username_URL }}
                        @else
                            {{ Auth::user()->name }} {{ Auth::user()->surname }}
                        @endif <br>
...
$(document).ready(function(){
  ...
  // $("#influencer_select").change(function(){
    ...
    } else if ($("#influencer_select").val() != "All influencers" && $("#influencer_select").val() != "") {

      window.location.href = "/user/influencer/"+$("#influencer_select").val()+"";
    ...
    afterSelect: function (item) {
        ...
        if (match) {
            ...
            $('.user_search').typeahead({
                ...
                afterSelect: function(item) {
                    // window.location.href = "/user/user/"+item.name+"";
     ...
     <!--   <script>
                ...
                //   //$(".drop-down-menu").css("transition","0.3s");
